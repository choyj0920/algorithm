#include <iostream>
#include <cstdio>
#include<vector>
#include <Algorithm>
#include<string>

using namespace std;
typedef  long long  ll;

struct  node
{
	int l, r, cnt, lazy;
}T[26][400000];

char S[100005];

int N, M;
int L, R, V;

int pull_up(int t, int k) {
	T[t][k].cnt = T[t][2 * k].cnt + T[t][2 * k + 1].cnt;

	return 0;
}

int push_down(int t, int k) {
	int lazy = T[t][k].lazy;
	if (lazy == 0) {
		T[t][2 * k].lazy = 0; T[t][2 * k + 1].lazy = 0;
		T[t][2 * k].cnt = 0; T[t][2 * k + 1].cnt = 0;
	}
	else if (lazy == 1) {
		T[t][2 * k].lazy = 1; T[t][2 * k + 1].lazy = 1;
		T[t][2 * k].cnt = T[t][2 * k].r - T[t][2 * k].l + 1;
		T[t][2 * k + 1].cnt = T[t][2 * k+1].r - T[t][2 * k+1].l + 1;

	}
	T[t][k].lazy = -1;

	return 0;
}

//t 트리번호, k 루트 , l,r 
int build(int t, int k, int L, int R) {
	int mid;
	T[t][k].l = L; T[t][k].r = R; T[t][k].lazy = -1;
	if (L == R) {
		if (S[L] - 'a' == t)  T[t][k].cnt = 1; else T[t][k].cnt = 0; 
		return 0;
	}

	mid = (L + R) / 2;
	build(t,k * 2, L, mid);
	build(t,k * 2+1, mid+1, R);

	pull_up(t, k);
	return 0;

}

int query(int t,int  k,int L, int R) {
	int mid, lret, rret;

	if (T[t][k].l >= L && T[t][k].r <= R) {
		return T[t][k].cnt;
	}
	if (T[t][k].l > R || T[t][k].r < L) {
		return 0;
	}
	push_down(t, k);
	mid = (T[t][k].l + T[t][k].r) / 2;
	lret = query(t,k * 2, L, R);
	rret = query(t, k * 2 + 1, L, R);
	return lret + rret;
}


int update(int t, int k, int L, int R, int V) {
	int mid;
	if (T[t][k].l >= L && T[t][k].r <= R) {
		T[t][k].lazy = V;
		if (V == 1) T[t][k].cnt = T[t][k].r - T[t][k].l + 1;
		else T[t][k].cnt = 0;
		return 0;
	}
	if (T[t][k].l > R || T[t][k].r < L) { return 0; }
	mid = (T[t][k].l + T[t][k].r) / 2;
	push_down(t, k);
	update(t, k*2, L, R, V);
	update(t, k * 2 + 1, L, R, V);
	pull_up(t, k);
	return 0;
}

int main() {
	int i;
	int c;
	int num[26];
	int left,right;
	cin >> N >> M;
	cin >> (S+1);

	for (i = 0; i < 26; i++) {
		build(i, 1, 1, N);
	}
	
	int x, y, d;
	for (int i = 1; i <= M; i++) {
		cin >> x >> y >> d;
		for (int j = 0; j < 26; j++) num[j] = 0;
		if (d == 1) {
			for (int j = 0; j < 26; j++) {
				num[j] = query(j, 1, x, y);
				update(j, 1, x, y, 0);
			}
			left = x;
			for (int j = 0; j < 26; j++) {
				if (num[j] > 0) {
					update(j, 1, left, left + num[j] - 1, 1);
					left = left + num[j];
				}
			}
		}
		else {
			for (int j = 0; j < 26; j++) {
				num[j] = query(j, 1, x, y);
				update(j, 1, x, y, 0);
			}
			left = x;
			for (int j = 25; j >= 0; j--) {
				if (num[j] > 0) {
					update(j, 1, left, left + num[j] - 1, 1);
					left = left + num[j];
				}
			}
		}
	}
	for (int i = 1; i <= N; i++) {
		for (int j = 0; j < 26; j++) {
			if (query(j, 1, i, i) > 0) {
				putchar('a' + j);
			}
		}
	}
	cout << '\n';
	
}